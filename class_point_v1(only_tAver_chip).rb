...
class ComputeData 
  ...
  def computeTemperature(arrayOrdered)
    ...
    tAverage = multi_T_dist / distance
    return tAverage.round(3), arraySingleTmean, distance
  end
  ...
  def writeFile(arrayOrderedWithDistances, arrayOfMeanTemp)
    File.open("output_only_chip.txt", 'w+') do |file|
      for i in (0...arrayOrderedWithDistances.length)
...
def main
  ...
  tAverage, arraySingleTmean, distance = computedData.computeTemperature(arrayOrderedWithDistances)
  
  computedData.printOrderedData(arrayOrderedWithDistances,arraySingleTmean)
  
  puts "CHIP"
  puts "T_Average_chip = #{tAverage}, C \ndistance_chip = #{distance.round(3)}, mm"
