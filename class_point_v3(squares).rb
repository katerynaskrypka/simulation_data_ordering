...
class ComputeData 
 ...
 # def writeFile(arrayOrderedWithDistances, arrayOfMeanTemp, index)
 #   File.open("output_chip" + index.to_s + ".txt", 'w+') do |file|
 #     for i in (0...arrayOrderedWithDistances.length)
...
class ComputeAllData 
  attr_accessor :limit_start, :limit_stop,:startChip, :stopChip,:startTool, :stopTool, :squareDim, :step
  def initialize()
    @arrayOfChip = Array.new()
    @arrayOfTempChip = Array.new()
    @arrayOfTool = Array.new()
    @arrayOfTempTool = Array.new()
    @step = 0.2
    @squareDim = 1
  ...
  def singleChip(newData1, actualStartLimit, actualStopLimit, index)  #newData1 =  CHIP
    ...
    arrayOrderedWithDistances = Array.new()
    ...
    for i in 0...arrayOrderedWithDistances.length
      @arrayOfChip.push(arrayOrderedWithDistances[i])
    end
    
    @arrayOfTempChip.push(arraySingleTmean)
    #computedData.writeFile(arrayOrderedWithDistances,arraySingleTmean, index)
  ...
  def singleTool(newData1, actualStartLimit, actualStopLimit, index) #newData1 =  TOOL
    ...
    for i in 0...arrayOrderedWithDistances.length
      @arrayOfTool.push(arrayOrderedWithDistances[i])
    end
    @arrayOfTempTool.push(arraySingleTmean)
  ...
  def printAllData()
    File.open("output_chip.txt", 'w+') do |file|
      for i in (0...@arrayOfChip.length)
        stringa = @arrayOfChip[i].x.to_s + "\t".to_s + @arrayOfChip[i].y.to_s + "\n".to_s 
       file.write(stringa)
      end
    end
    
    File.open("output_tool.txt", 'w+') do |file|
      for i in (0...@arrayOfTool.length)
        stringa = @arrayOfTool[i].x.to_s + "\t".to_s + @arrayOfTool[i].y.to_s + "\n".to_s 
       file.write(stringa)
      end
    end
