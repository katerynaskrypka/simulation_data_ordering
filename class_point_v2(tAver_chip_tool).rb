...
class ComputeData 
  ...
  def computeSingleDistance(firstPoint, secondPoint)
    return distance_chip = Math::sqrt((secondPoint.x - firstPoint.x)**2 + (secondPoint.y - firstPoint.y)**2)
  end
...
def readData 
  ...
  File.readlines(unsorted).each_with_index do |line, i| 
    words = line.split(",") 
    p1 = PointTool.new(index, words[0], words[1], words[2])
    arrayOfData.push(p1)
    index += 1
  end
  return arrayOfData
...
class ComputeData 
  attr_accessor :limit_1, :limit_2, :index_deleted, :distance_tool
  def initialize()
    @limit_1 = 3.6
    @limit_2 = 4.6
    @index_deleted = Array.new()
  ...
  def computeTemperature(arrayOrdered)
    ...
    for i in (0...arrayOrdered.length - 1)
      ...
      arraySingleTmean.push(singleTmean)
      singleDistance = computeSingleDistance(arrayOrdered[i], arrayOrdered[i + 1])
      distance_tool += singleDistance
      multi_T_dist = singleTmean * singleDistance + multi_T_dist
    end
    tAverage_tool = (multi_T_dist / distance_tool).round(3)
    return tAverage_tool, arraySingleTmean, distance_tool.round(3)
  end
  ...
  tAverage_tool_chip = ( tAverage_tool * distance_tool + tAverage_chip * distance_chip) / (distance_chip + distance_tool)
 
  puts "\nTOOL/CHIP"
  puts "T_Average_tool_Chip = (#{tAverage_chip} * #{distance_chip} + #{tAverage_tool} * #{distance_tool}) / (#{distance_chip} + #{distance_tool})"
  puts "T_Average_tool_Chip = #{tAverage_tool_chip.round(2)}, C"
